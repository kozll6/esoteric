...
struct QuantumSuperpositionDelayWidget : ModuleWidget {
	QuantumSuperpositionDelayWidget(QuantumSuperpositionDelay* module) {
		...
		// Parameters (left column) - adjusted to fit 128.5mm panel
		float knobX = 15.f;
		float knobY = 28.f;
		float knobSpacing = 16.5f;
		...
		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(knobX, knobY + knobSpacing * 4)), module, QuantumSuperpositionDelay::MIX_PARAM));
		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(knobX, knobY + knobSpacing * 5)), module, QuantumSuperpositionDelay::CHAOS_PARAM));

		// CV Inputs (right column)
		float cvX = 40.f;
		float cvY = 28.f;
		float cvSpacing = 13.f;
		...
		// Output
		addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(cvX, 93.5f)), module, QuantumSuperpositionDelay::AUDIO_OUTPUT));

		// Lights
		float lightX = 40.f;
		float lightY = 105.f;
		float lightSpacing = 5.f;
		...
		for (int i = 0; i < QuantumSuperpositionDelay::NUM_BUFFERS; i++) {
			addChild(createLightCentered<SmallLight<BlueLight>>(mm2px(Vec(lightX + (i % 3) * lightSpacing, lightY + 8.f + (i / 3) * lightSpacing)), module, QuantumSuperpositionDelay::BUFFER_LIGHT_1 + i));
		}
	}
};
